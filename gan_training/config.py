...
from gan_training.models import generator_dict, discriminator_dict, video_discriminator_dict, text_encoder_dict
...
def build_models(config):
    ...
    ImageDiscriminator = discriminator_dict[config['discriminator']['name']]
    VideoDiscriminator = video_discriminator_dict[config['discriminator']['name']]
    TextEncoder = text_encoder_dict[config['discriminator']['name']]
    # Build models
    generator = Generator(
    ...
    text_encoder = TextEncoder(z_dim=config['z_dist']['dim'])

    return generator, image_discriminator, video_discriminator, text_encoder


def build_optimizers(generator, image_discriminator, video_discriminator, text_encoder, config):
    ...
    toogle_grad(image_discriminator, True)
    toogle_grad(video_discriminator, True)
    toogle_grad(text_encoder, True)
    ...
    if equalize_lr:
        ...
        d_params = image_discriminator.parameters()
        vd_params = video_discriminator.parameters()
        enc_params = text_encoder.parameters()
    ...
    if optimizer == 'rmsprop':
        ...
        vd_optimizer = optim.RMSprop(vd_params, lr=lr_d, alpha=0.99, eps=1e-8)
        enc_optimizer = optim.RMSprop(enc_params, lr=lr_d, alpha=0.99, eps=1e-8)
    elif optimizer == 'adam':
        ...
        vd_optimizer = optim.Adam(vd_params, lr=lr_d, betas=(0., 0.99), eps=1e-8)
        enc_optimizer = optim.Adam(enc_params, lr=lr_d, betas=(0., 0.99), eps=1e-8)
    elif optimizer == 'sgd':
        g_optimizer = optim.SGD(g_params, lr=lr_g, momentum=0.)
        d_optimizer = optim.SGD(d_params, lr=lr_d, momentum=0.)
        vd_optimizer = optim.SGD(vd_params, lr=lr_d, momentum=0.)
        enc_optimizer = optim.SGD(enc_params, lr=lr_d, momentum=0.)

    return g_optimizer, d_optimizer, vd_optimizer, enc_optimizer
