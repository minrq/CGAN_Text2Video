...
class ImageDiscriminator(nn.Module):
    def __init__(self, z_dim, size, embed_size=64, nfilter=32, **kwargs):
        ...
        # stage nets
        self.stage1_in_transform = nn.Conv2d(4*nf, 8, 1, 1, 0)
        self.stage2_in_transform = nn.Conv2d(4*nf, 8, 1, 1, 0)
    ...
    def forward(self, x, y_filt, y_encoding):
        ...
        y_filt_stage1 = y_filt_ex[:, 0:8, :, :, :]
        y_filt_stage2 = y_filt_ex[:, 8:, :, :, :]
...
class VideoDiscriminator(nn.Module):
    def __init__(self, z_dim, size, embed_size=64, nfilter=32, **kwargs):
        ...
        # stage nets
        self.stage1_in_transform = nn.Conv3d(4*nf, 8, 1, 1, 0)
        self.stage2_in_transform = nn.Conv3d(4*nf, 8, 1, 1, 0)
        ...
        self.stage1_net = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.AvgPool3d(4, 4),
            nn.Conv3d(8, self.nemb, (1, 2, 2), 1, 0)
        )
    ...
    def forward(self, x, y_filt, y_encoding):
        ...
        y_filt_stage1 = y_filt[:, 0:8, :, :, :, :]
        y_filt_stage2 = y_filt[:, 8:, :, :, :, :]
